...
public class DataStreamJob {
	...
	public static void main(String[] args) throws Exception {
		...
		transactionStream.map(
			transaction -> {
				Date transationDate = new Date(System.currentTimeMillis());
				String category = transaction.getProductCategory();
				double totalSales = transaction.getTotalAmount();
				return new SalesPerCategory(transationDate, category, totalSales);
			}
		...
		transactionStream.map(
			transaction -> {
				Date transationDate = new Date(System.currentTimeMillis());
				double totalSales = transaction.getTotalAmount();
				return new SalesPerDay(transationDate, totalSales);
			}
		...
		transactionStream.map(
			transaction -> {
				Date transationDate = new Date(System.currentTimeMillis());
				int month = transationDate.toLocalDate().getMonth().getValue();
				int year = transationDate.toLocalDate().getYear();
				double totalSales = transaction.getTotalAmount();
